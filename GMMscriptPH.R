...
#0. Install necessary packages & load required libraries
...
find_best_GMM_silhouette <- function(data, max_clusters = 6, enforce_G = NULL) {
  ...
  #Enforce a specific number of clusters to have 3 categories (e.g., G = 3)
...
#cluster        p        s         v                 label
#1       1 1.788329 1.797356 1.4668999     Highly Vulnerable
#2       2 2.442740 1.775831 0.9721936 Moderately Vulnerable
#3       3 2.671248 1.529764 0.6313251      Least Vulnerable
...
#10. Export table with species and assigned cluster
...
ggplot(PSAdata, aes(x = p, y = s, shape = label)) +
  ...
  stat_ellipse(aes(group = label), linetype = "dashed", linewidth = 1, alpha = 0.2) +
